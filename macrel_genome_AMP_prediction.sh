...
# Directory where your genome FASTA files are located
input_dir="genomes"
# Directory where Macrel results will be stored
output_dir="macrel_output"
# Directory where consolidated results from all genomes will be stored
...
echo -e "Genome\tAccess\tSequence\tAMP_Family\tAMP_Probability\tHemolytic\tHemolytic_Probability\tNonHemolytic_Probability" > $all_peptides
...
for fasta in $input_dir/*.fasta; do
  ...
  # Define the output directory for each genome
  ...
  if [ $? -eq 0 ]; then
    ...
    # Check if compressed prediction file exists
